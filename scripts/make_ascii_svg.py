...
(no JS needed). Each row is revealed with a left-to-right clip wipe plus a
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "afiq-ascii.svg")
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{PROMPT}:~$ whoami <tspan fill="{INK}">{WHOAMI}</tspan></text>')

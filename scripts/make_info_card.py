...
portrait: colored key/value rows for status, education, tech stack, and
highlights -- NOT GitHub stats (the contribution graph covers those).

Static content, hand-authored below. Lines fade/slide in on a short stagger
so it feels like the panel is printing alongside the portrait. STATIC=1 emits
the frozen state for Quick Look previews.
"""
import html
import os
...
ROWS = [
    ("host",),
    ("kv", "Now", "Final-year CS (Data Eng) @ UTM"),
    ("kv", "Next", "Data Eng Intern @ PPG Coatings (Aug '26)"),
    ("kv", "Edu", "B.CS Data Engineering, UTM · CGPA 3.94"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Data", "PySpark, Polars, Pandas, Dask"),
    ("kv", "Cloud", "Azure (ADF/Databricks/Synapse), AWS"),
    ("kv", "App", "Flutter, Firebase, React, Supabase"),
    ("kv", "BI", "Alteryx, Tableau, Power BI"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Built an Azure medallion pipeline, ADF -> Synapse"),
    ("bul", "Scraped 150k+ records w/ Playwright + Polars"),
    ("bul", "AWS Cloud Practitioner + Azure DP-900 certified"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">afiq@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">afiq</tspan><tspan fill="{MUTED}">@</tspan>'
